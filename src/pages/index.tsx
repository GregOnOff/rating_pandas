...
export default function Home() {
    ...
    `;
        const mutateData = await directus.graphql.items(mutation)
    ...
    function createPanda() {
        directus.items('pages').createOne({
            name_input: 'Panda1'
        })
            .then(response => {
  ...
  `;
        ...
        const mutateData = await directus.graphql.items(updateMutation, variables)
    ...
    <main className="flex min-h-screen flex-col items-center justify-between p-24 ">
      <h1>Rate pictures of pandas</h1>
        ...
        <input type={"text"} name={'updatedName'}/>
            ...
            <button className='m-4 p-4 rounded-xl bg-gray-400' type={'submit'}>Submit</button>
        </form>
        {/*<p>Status: {directusData?[0].rating}</p>*/}
        <ImageCard directusData={directusData} setDirectusData={setDirectusData}/>
